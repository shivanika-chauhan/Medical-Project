...
const Medicines = () => {
  ...
  return (
    <div className="w-full bg-gradient-to-b from-blue-50 to-green-50 min-h-screen">
      {/* Hero Section */}
      <section className="w-full text-center py-12 px-4 bg-gradient-to-r from-blue-600 to-green-500 text-white shadow-md">
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-12 py-8">
        {categories.map((category) => {
          ...
          return (
            <div
              key={category}
              className="mb-8 sm:mb-12 border rounded-lg shadow-md bg-white"
            >
              {/* Category Header */}
              <div
                className="bg-gray-100 p-4 sm:p-5 cursor-pointer flex justify-between items-center rounded-t-lg"
                ...
                <div className="p-4 sm:p-6">
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 sm:gap-6">
                    {itemsToShow.map((item) => (
                      <div
                        key={item.id}
                        className="border rounded-xl bg-white shadow-sm hover:shadow-lg hover:scale-105 transition-transform duration-300 p-3 sm:p-4 flex flex-col items-center text-center"
                      >
                        <img
                          src={item.image}
                          alt={item.name}
                          className="h-24 sm:h-28 md:h-32 w-auto mb-3 object-contain"
                          loading="lazy"
                        />
                        <h3 className="text-xs sm:text-sm md:text-base font-semibold text-gray-700 line-clamp-2 mb-2">
                          {item.name}
                        </h3>

                        {/* Price & Add to Cart */}
                        <div className="flex flex-col sm:flex-row justify-between items-center w-full border-t pt-2 mt-auto gap-2">
                          <p className="text-green-600 text-sm sm:text-base font-medium">
                            ₹{item.price.toLocaleString("en-IN")}
                          </p>
                          <button
                            onClick={() => addToCart(item)}
                            className="w-full sm:w-auto px-2 py-1 sm:px-3 sm:py-2 bg-blue-600 text-white text-xs sm:text-sm rounded-md hover:bg-blue-700 active:scale-95 transition shadow-md"
                          >
                            Add
                          </button>
                        </div>
                  ...
                  {categoryItems.length > 10 && (
                    <div className="mt-4 text-center">
                      <button
                        className="px-4 py-2 bg-blue-600 text-white rounded-md text-sm sm:text-base hover:bg-blue-700 transition"
      ...
      {/* Info Section */}
      <section className="bg-white shadow-lg rounded-xl max-w-7xl mx-auto px-4 sm:px-6 py-8 mb-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 sm:gap-8 text-center">
          ...
          <div className="p-4 border rounded-lg hover:shadow-md transition">
            ...
            <p className="text-gray-700 text-sm sm:text-base">
              Find medicines for various health conditions, from common ailments
              to specialized needs.
            </p>
          </div>
          <div className="p-4 border rounded-lg hover:shadow-md transition">
            <h3 className="text-lg sm:text-xl font-semibold mb-2">Trusted Brands</h3>
            <p className="text-gray-700 text-sm sm:text-base">
